...
<div class="max-w-[85rem] px-4 sm:px-6 lg:px-8 mx-auto">
    <div class="grid lg:grid-cols-3 gap-y-8 lg:gap-y-0 lg:gap-x-6">
      ...
      <div class="lg:col-span-2">
        <div class="py-8 lg:pe-8">
          <div class="space-y-5 lg:space-y-8">
            <a class="inline-flex items-center gap-x-1.5 text-sm text-gray-600 decoration-2 hover:underline dark:text-blue-500" href="{{ route('publik.publik-posts') }}">
              <svg class="flex-shrink-0 size-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m15 18-6-6 6-6"/></svg>
              Back to Blog
            </a>

            <h2 class="text-3xl font-bold lg:text-5xl dark:text-black">{{ $posts->title }}</h2>
            ...
            <div class="flex items-center gap-x-5">
              ...
              <p class="text-xs text-gray-800 sm:text-sm dark:text-gray-800">{{ $posts->tanggal }}</p>
            </div>

            <figure>
                <img class="object-cover w-full rounded-xl" src="{{ asset('storage/images/'.$posts->image) }}" alt="Image Description">
                <figcaption class="mt-3 text-sm text-center text-gray-500 dark:text-gray-800">
                  Penulis: {{ $posts->penulis }}
                </figcaption>
            </figure>

            <p class="text-lg text-gray-800 dark:text-gray-800">{{ $posts->isi }}</p>
      ...
      <!-- Sidebar -->
      <div class="sticky py-8 top-10 start-0 lg:ps-8">
        <div class="sticky py-8 top-10 start-0 lg:ps-8">
          <div class="space-y-6">
            <!-- Media -->
            @foreach ($relatedPosts as $relatedPosts)
            @if ($relatedPosts->id !== $posts->id)
            <hr style="background-color: rgb(100, 100, 100); height: 2px;">
            <a class="flex items-center group gap-x-6" href="{{ route('publik.publik-show-posts', $relatedPosts->id) }}">
                <div class="grow">
                  <span class="text-sm font-bold text-gray-800 group-hover:text-black dark:text-gray-800 dark:group-hover:text-black">
                    {{ $relatedPosts->title }}
                  </span>
                </div>
