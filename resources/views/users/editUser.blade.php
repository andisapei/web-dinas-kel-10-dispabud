<x-app-layout>
   ...
   <!-- Content -->
<div class="w-full px-2 pt-4 sm:px-6 md:px-8 lg:ps-72">
    <!-- Card Section -->
    ...
    <div class="p-4 bg-white shadow rounded-xl sm:p-7 dark:bg-neutral-800">
      ...
      <form action="{{ route('users.update', $user->id) }}" method="post" enctype="multipart/form-data">
        ...
        <div class="grid gap-2 sm:grid-cols-12 sm:gap-6">
          ...
          <div class="sm:col-span-9">
            <div class="flex items-center gap-5">
              <img class="inline-block rounded-full size-16 ring-2 ring-white dark:ring-neutral-900" src="{{ asset('storage/images/'.$user->profile) }}" alt="Image Description">
          ...
          <div class="sm:col-span-9">
            <div class="space-y-2">
              ...
              <span class="text-sm text-red-600">Kosongkan Untuk Tidak Merubah Password</span>
              @error('password')
              <span class="text-sm text-red-600">{{ $message }}</span>
              @enderror
            </div>
        ...
        <div class="flex justify-end mt-5 gap-x-2">
          <a href="{{ route('users.index') }}" onclick="return confirm('back to User?')" class="inline-flex items-center px-3 py-2 text-sm font-medium text-gray-800 bg-white border border-gray-200 rounded-lg shadow-sm gap-x-2 hover:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-800 dark:border-neutral-700 dark:text-white dark:hover:bg-neutral-800">
            Cancel
          </a>
